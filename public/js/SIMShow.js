$(document).ready(function(){
	cont = $("#cont").val();//CONTADOR PARA LAS IMAGENS
    $('#mostrar').hide();
...
function verificar_imagen(){
	//cont++;
	for (var i = cont; i < 11; i++) {
		$("#div_img"+i).hide();
		document.getElementById("list_"+i).innerHTML ="";  
		$("#imagen_"+i).val("");				
	}
}
...
function Validar_SIM_Guardar(){
	if ($('select[name=empresa]').val() == 0 || $('select[name=granja]').val() == 0 || $('input[name=zona]').val() == "" || $('select[name=edad]').val() == 0 || $('select[name=sexo]').val() == "" || $('input[name=nro_pollos]').val() == "" || $('input[name=fecha]').val() == "") { //|| $('#detalle').val() == ""  || $('input[name=fechaFin]').val() == "" || $('select[name=carrera]').val() == 0 || $('input[name=fechaLimite]').val() == "" || $('input[name=cupos]').val() == "" || $('select[name=tipo]').val() == "" || $('#grado').val() == 0 || $('#ciudad').val() == 0 || $('#imagen').val() == "") {
	  ...
	  if ($('select[name=edad]').val()==0) {alertify.error('No introdujo el campo Edad'); $('#div_edad').addClass('has-error');}else{$('#div_edad').removeClass('has-error');}		
